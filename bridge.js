...
var port = phantom.args[0] // eslint-disable-line
...
function setupPushNotifications (id, page) {
  var callbacks = ['onAlert', 'onConfirm', 'onConsoleMessage', 'onError', 'onInitialized', 'onLoadFinished', 'onLoadStarted', 'onPrompt', 'onResourceRequested', 'onResourceReceived', 'onResourceError', 'onUrlChanged', 'onCallback']
  ...
  if (request[0] === 0) {
    switch (request[2]) {
      case 'createPage':
        ...
        break
      case 'injectJs':
        var success = phantom.injectJs(request[3]) // eslint-disable-line
        respond([0, cmdId, 'jsInjected', success])
        break
      case 'addCookie':
        phantom.addCookie(request[3]) // eslint-disable-line
        respond([0, cmdId, 'cookieAdded', success])
        break
      case 'exit':
        // optimistically to get the response back before the line is cut
        respond([0, cmdId, 'phantomExited'])
        break
      case 'exitAck':
    ...
    switch (request[2]) {
      ...
      case 'pageGet':
        ...
        break
      case 'pageSetFn':
        page[request[3]] = eval('(' + request[4] + ')') // eslint-disable-line
        break
